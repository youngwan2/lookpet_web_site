...
router.post('/auth/idcheck', (req, res) => {
...
router.post('/auth/nickcheck', (req, res) => {
  const { nickname } = req.body
  userModel.findOne({ nickname: nickname }).then((result) => {
    if (result) {
      res.json({ message: '중복된 닉네임입니다.', success: false })
    } else {
      res.json({ message: '생성가능한 닉네임입니다.', success: true })
    }
  })
})
