...
router.post("/board/:id/like-user", (req, res) => {
  ...
  const { postId, username, like, unlike } = req.body;
      ...
      if (!findResult) {
        likeUserModel
          ...
          .then((result) => {
            ...
            if (like) {
              likeCounterModel.insertMany({ postId: postId, like: 1 });
            }
            if (unlike) {
              likeCounterModel.insertMany({ postId: postId, unlike: 1 });
            }
        ...
        //해당 게시글에 유저 정보가 존재하지 않는다면 실행
      } else {
        likeUserModel
          .findOne({ $and: [{ postId: postId, username: username }] })
          .then((likeUserResult) => {
            console.log("존재하는 게시글을 찾음:", likeUserResult);
            likeCounterModel.findOne({ postId: postId }).then((findPost) => {
              const { postId, like, unlike } = likeUserResult;
              let { likeCount, unlikeCount } = findPost;

              // true 라면 좋아요를 1 증가 시킨다.
              if (like) {
                likeCounterModel.updateOne({postId:postId},{like:likeCount++})
              }
              if(unlike) {
                likeCounterModel.updateOne({postId:postId},{unlike:unlikeCount++})
              }
            });
